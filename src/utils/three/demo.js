...
import vertexShader from '@/utils/three/shader/FlyLine/vertex.glsl';
import fragmentShader from '@/utils/three/shader/FlyLine/fragment.glsl';
...
export function init() {
  ...
  initFlyLine(
    {
      getPoints(pointsNumber) {
        const points = genPointsBetweenTwoPoints([0, 0, 0], [10, 10, 0], pointsNumber);
...
function initCircleCurveGroup(number) {
  ...
  for (let i = 0; i < number; i++) {
    let curve = new THREE.EllipseCurve(
      ...
      0, // ax, aY
      Math.random() * 20 + 5,
      Math.random() * 20 + 5, // xRadius, yRadius
      0,
      2 * Math.PI, // aStartAngle, aEndAngle
      false, // aClockwise
      0 // aRotation
